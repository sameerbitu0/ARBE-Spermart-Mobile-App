...
class LoginBloc extends Bloc<LoginEvent, LoginModelState> {
  ...
  LoginBloc({
    ...
    on<LoginEventAccountDelete>(_accountDelete);
  ...
  Future<void> _accountDelete(
    LoginEventAccountDelete event,
    Emitter<LoginModelState> emit,
  ) async {
    emit(state.copyWith(state: const LoginStateAccountDeleteLoading()));

    final result = await _authRepository.accountDelete(userInfo!.accessToken);

    result.fold(
      (Failure failure) {
        if (failure.statusCode == 500) {
          const loadedData = LoginStateAccountDelete('logout success', 200);
          emit(state.copyWith(state: loadedData));
        } else {
          final error =
              LoginStateSignOutError(failure.message, failure.statusCode);
      ...
      (String success) {
        _user = null;
        final loadedData = LoginStateAccountDelete(success, 200);
        emit(state.copyWith(state: loadedData));
      },
    );
  }
